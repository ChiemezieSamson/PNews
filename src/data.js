import pix4 from "../src/asset/images/pix4.jpg";
import pix1 from "../src/asset/images/pix1.jpg";
import pix2 from "../src/asset/images/pix2.jpg";
import pix3 from "../src/asset/images/pix3.jpg";
import pix5 from "../src/asset/images/pix5.jpg";
import pix6 from "../src/asset/images/pix6.jpg";
import pix7 from "../src/asset/images/pix7.jpg";
import pix8 from "../src/asset/images/pix8.jpg";
import pix9 from "../src/asset/images/pix9.jpg";
import pix10 from "../src/asset/images/pix10.jpg";
import pix11 from "../src/asset/images/pix11.jpg";
import pix12 from "../src/asset/images/pix12.jpg";
import pix13 from "../src/asset/images/pix13.jpg";
import {
	FaFacebook,
	FaFlickr,
	FaInstagram,
	FaLinkedin,
	FaRss,
	FaSoundcloud,
	FaTwitter,
	FaYoutube,
} from "react-icons/fa";

export const Posts = [
	{
		id: 1,
		image: pix1,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit 1",
		cat: "Fashion",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 20, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 2,
		image: pix2,
		title: "Omnis autem, magnam consequuntur accusamus 2",
		cat: "App",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 21, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 3,
		image: pix3,
		title: "Ab tenetur exercitationem quos ipsum eum ducimus quas earum  3",
		cat: "Business",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 22, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 4,
		image: pix4,
		title: "Ipsum dolor sit amet consectetur adipi elit Health 4",
		cat: "Health",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 23, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 5,
		image: pix5,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit 5",
		cat: "World",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 24, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 6,
		image: pix6,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit 6",
		cat: "Food",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 25, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 7,
		image: pix7,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Tech",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 26, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 8,
		image: pix8,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Life",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 27, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 9,
		image: pix9,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Books",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 28, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 10,
		image: pix10,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Travel",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 29, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 11,
		image: pix11,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Movies",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 30, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 12,
		image: pix12,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Words",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "June 31, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
	{
		id: 13,
		image: pix13,
		title: "Lorem ipsum dolor sit amet consectetur adipi elit",
		cat: "Action",
		content:
			"Lorem ipsum dolor sit amet consectetur adipisicing elit. Ea, omnis autem, magnam consequuntur accusamus ab tenetur exercitationem quos ipsum eum ducimus quas earum. Sint eveniet pariatur nisi, dicta porro animi!",
		date: "July 01, 2022",
		tag: ["Fitness Tips", "Women's Health"],
	},
];

export const ParentCategories = {
	books: {
		maintags: ["fashion", "food", "health", "travel"],
		trendingtag: [
			"trump Inauguration",
			"United Stated",
			"election results",
			"white House",
		],
	},
	lifestyle: {
		maintags: ["gaming", "movie", "music", "sports"],
		trendingtag: [
			"nintendo switch",
			"ces 2017",
			"mark zuckerberg",
			"playstation 4 pro",
			"sillicon valley",
			"internet",
		],
	},
	favorite: {
		maintags: ["apps", "gadget", "mobile", "startup"],
		trendingtag: [
			"golden globes",
			"game of thrones",
			"motoGp 2017",
			"esport",
			"fashion week",
			"jorge lorenzo",
		],
	},
	business: {
		maintags: ["business", "politics", "science", "world"],
		trendingtag: [
			"golden globes",
			"game of thrones",
			"motoGp 2017",
			"election results",
		],
	},
	random: {
		maintags: ["news", "review", "entertainment", "featured"],
		trendingtag: [
			"nintendo switch",
			"ces 2017",
			"mark zuckerberg",
			"playstation 4 pro",
			"esport",
			"fashion week",
		],
	},
};

export const cats = [
	"apps",
	"business",
	"entertainment",
	"fashion",
	"featured",
	"food",
	"gadget",
	"gaming",
	"health",
	"lifestyle",
	"mobile",
	"movie",
	"music",
	"news",
	"politics",
	"review",
	"science",
	"sports",
	"startup",
	"tech",
	"travel",
	"world",
];

export const SocialMediaIcons = [
	{
		id: 1,
		icon: <FaInstagram />,
	},
	{
		id: 2,
		icon: <FaFacebook />,
	},
	{
		id: 3,
		icon: <FaTwitter />,
	},
	{
		id: 4,
		icon: <FaYoutube />,
	},
	{
		id: 5,
		icon: <FaLinkedin />,
	},
];

export const HomeAndPigSocialLinks = [
	{
		id: 1,
		icon: <FaInstagram className="text-white inline-block" />,
		number: 87.1 + "k",
		text: "Followers",
		bg: "bg-[#e4405f]",
		hv: "hover:bg-[#e4405f]/70",
	},
	{
		id: 2,
		icon: <FaFlickr className="text-white inline-block" />,
		number: 649,
		text: "Followers",
		bg: "bg-[#f07]",
		hv: "hover:bg-[#f07]/70",
	},
	{
		id: 3,
		icon: <FaSoundcloud className="text-white inline-block" />,
		number: 23.7 + "k",
		text: "Followers",
		bg: "bg-[#ffae00]",
		hv: "hover:bg-[#ffae00]/70",
	},
	{
		id: 4,
		icon: <FaRss className="text-white inline-block" />,
		number: 99,
		text: "Subscribers",
		bg: "bg-[#ff6f00]",
		hv: "hover:bg-[#ff6f00]/70",
	},
];

export const Map = `
		<iframe 
		src="https://www.google.com/maps/embed?pb=!1m14!1m12!1m3!1d5609.90112266805!2d6.832326377011093!3d6.157652257901957!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!5e0!3m2!1sen!2sfr!4v1670209134362!5m2!1sen!2sfr" 
		width="100%" 
		height="450" 
		style="border:0;" 
		allowfullscreen="" 
		loading="lazy" 
		referrerpolicy="no-referrer-when-downgrade"
		></iframe>`;

export const tags = [
	{
		id: 1,
		tag: "bitcoin",
	},
	{
		id: 2,
		tag: "champions league",
	},
	{
		id: 3,
		tag: "explore bali",
	},
	{
		id: 4,
		tag: "golden globes 2018",
	},
	{
		id: 5,
		tag: "grammy awards",
	},
	{
		id: 6,
		tag: "harboinas",
	},
	{
		id: 7,
		tag: "litecoin",
	},
	{
		id: 8,
		tag: "market stories",
	},
	{
		id: 9,
		tag: "united stated",
	},
];

export const User = {
	name: "",
	email: "",
	bio: "",
	profileImage: "",
	socialLinks: {
		twitter: "",
		facebook: "",
		linkedin: "",
		github: "",
	},
	password: "",
	confirmPassword: "",
	website: "",
	location: "",
};

// const [user, setUser] = useState();

// const handleInputChange = e => {
//   setUser({
//     ...user,
//     [e.target.name]: e.target.value
//   });
// };

// const handleSocialLinksChange = e => {
//   setUser({
//     ...user,
//     socialLinks: {
//       ...user.socialLinks,
//       [e.target.name]: e.target.value
//     }
//   });
// };

export const navItems = [
	{
		id: 1,
		url: "/books",
		name: "Books",
	},
	{
		id: 2,
		url: "/lifestyle",
		name: "Lifestyle",
	},
	{
		id: 3,
		url: "/favorite",
		name: "Favorite",
	},
	{
		id: 4,
		url: "/business",
		name: "Business",
	},
	{
		id: 5,
		url: "/quotes",
		name: "Quotes",
	},
];
