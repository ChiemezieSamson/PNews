    ...
    <section className='pb-10'>
      ...
      <ul className="md:grid md:grid-cols-2 pt-2">
        {Posts.slice(11, 13).map((post) => {
          return (
            <li key={post.id} className="md:first:mr-[3%] md:last:ml-[3%] first:mb-7 md:first:mb-0">
              <div className="relative mt-1.5 mb-4">
                <img src={post.image} alt={"game"} className="w-full h-44 imgxs:h-[240px] object-cover cursor-pointer md:h-36 lg:h-48 xl:h-[250px]" loading="lazy"/>
      ...
      <ul className='mt-7'>
        {Posts.slice(4, 8).map((post) => {
          return (
          <li key={post.id} className="grid imgxs:grid-cols-5 grid-cols-3 mb-3.5">
            <div className="my-2 imgxs:col-span-2 col-span-1 mr-[3%]">
              <img src={post.image} alt={"game"} className="w-full imgxs:h-36 md:min-h-[140px] lg:min-h-[176px] xl:min-h-full object-cover cursor-pointer" loading="lazy"/>
            </div>

            <div className='pt-1 imgxs:col-span-3 col-span-2 md:max-w-md text-black ml-[3%]'>
             {size.width > 768 ? <PostTitleMedium2 post={post.title} /> : <span className='pt-2 inline-block'> <PostTitleSmall post={post.title} /> </span>}
              ...
              {size.width > 519 && <PostsShortInfoComponent post={post.content} />}
            </div>
