...
const PreviousNextPost = ({post, User, useraction, userContent, size}) => {
  const {data: posts = [], isFetching, isSuccess} = useGetPostsByQueryQuery(`?category=${post?.postCategory[0]}`)
        ...
        useraction ?
          ...
          <section className='lg:grid-cols-6 lg:grid p-7 mb-7 border border-solid border-neutral-200 font-poppins text-center sm:text-left rounded-sm'>
            ...
            <div className='col-span-5'>
              <Link to={`/categories?user=${User?._id}`} className='mb-2.5 inline-block text-lg font-bold'>
                <span className='text-[#f70d28] cursor-pointer capitalize'>{User.username}</span>
              </Link>

              <p className='text-stone-700 font-josefin leading-5 mb-4'>
                {User.bio}
              </p>
